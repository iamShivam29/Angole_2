...
class WebShowsFragment : Fragment(), CategoryListItemAdapter.OnMainClick, MainRecyclerAdapter.OnClickEvent {
    ...
    private var seriesData = listOf<CategoryData>()
                ...
                if (it.data != null){
                    ...
                    if (subCode == 200){
                        if(it.data.items != null) {
                            ...
                            seriesData = it.data.items!!.categoryData!!
                        }
...
//                            binding?.rvWebShows?.visibility = View.VISIBLE
...
//                            binding?.mainItems?.visibility = View.GONE
                            ...
                            binding?.rvWebShows?.visibility = View.GONE
                            ...
                            streamViewModel?.webData?.removeObservers(viewLifecycleOwner)
                            streamViewModel?.webData?.value = null
                        }
                    }else if (subCode == 404) {
//                        Toast.makeText(requireContext(), "Data Not found", Toast.LENGTH_SHORT).show()
//                        binding?.mainItems?.visibility = View.GONE
...
//                streamViewModel?.webData?.value = null
    ...
    private fun loadMyListData(){
        ...
        val token = AuthConfig(requireContext()).getToken()
        val authToken = "Bearer $token"
        streamViewModel?.getMyShowsList(authToken)

        streamViewModel?.myShowListData?.observe(viewLifecycleOwner){
            it?.let {
                if (it.data != null) {
                    val subCode = it.data.subCode
                    if (subCode == 200) {
...
//        startActivity(intent)
    ...
    override fun onItemClick(categoryData: CategoryData) {
        ...
        intent.putExtra("from", Constants.FROM_SERIES)
        startActivity(intent)
    }
}